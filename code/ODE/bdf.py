...
def get_backward_difference(order):
	"""
	Get one specific order of backward difference.
	It just uses the backward_differences function, and grabs the 
	correct order. Probably not the most efficient but ok for now.
	"""
...
def bdf_coefficients_and_differences(T,order):
	...
	eta = np.prod([T[m]-T[m-i] for i in xrange(1,m)])/np.sum(1./(T[m] - T[m-j]) for j in xrange(1,m+1))
	#print(np.sum(1./(T[m] - T[m-j]) for j in xrange(1,m+2)))
	#print(T)
	#for i in xrange(1,m+1):
	#	print(T[m] - T[m-i])
	#print(eta)
	#exit()
	return [np.sum(np.prod([T[m]-T[m-i] for i in xrange(1,j)])*differences[j] for j in xrange(1,order+1)), differences,eta]
...
#	print(1/(np.prod([T[m]-T[m-i] for i in xrange(1,j)])))
