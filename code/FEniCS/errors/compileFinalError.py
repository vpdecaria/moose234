...
def getWorkData(filename):
	...
	while True:
		...
		pressure_error=float(data[8])/float(data[6])
		vel_error=float(data[7])/float(data[5])
		final_pressure_error=pressure_error
		final_vel_error=vel_error
#			print(final_pressure_error)
		counter += 1
	f.readline()
	line = f.readline()
	data = line.strip().split(',')
	...
	f.close()
	return [counter,l2L2,l2H1,l2L2Pressure,dt,final_pressure_error, final_vel_error]
...
def compileErrors(fName,dtList):
	...
	dts = np.array([])
	rejections = np.array([])
	final_pressure_errors=np.array([])
	final_vel_errors=np.array([])
	for dt in dtList:
		[countertemp,l2L2temp,l2H1temp,l2L2Pressuretemp,currentDt,final_pressure_error, final_vel_error] = getWorkData(fName + str(dt)+'.txt')
		...
		final_pressure_errors = np.append(final_pressure_errors,final_pressure_error)
		final_vel_errors = np.append(final_vel_errors,final_vel_error)
	return [counter,l2L2,l2H1,l2L2Pressure,dts,final_vel_errors,final_pressure_errors]

def writeErrorsToFile(fName,dtList,outputfname):
	[counter,l2L2,l2H1,l2L2Pressure,dts,final_vel_error,final_pressure_error] = compileErrors(fName,dtList)
	f = open(outputfname,'w')
	f.write('Junk\n')
	f.write('dt, l2L2 error, l2H1 error, l2L2 Pressure error, Final Velocity Error,Final Pressure Error \n')
	for i, dt in enumerate(dts):
		output = "\n" + str(dt) + "," + str(l2L2[i]) + "," +str(l2H1[i])  + "," + str(l2L2Pressure[i]) + "," + str(final_vel_error[i]) +"," + str(final_pressure_error[i])
		f.write(output)
