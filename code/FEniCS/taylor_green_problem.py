...
V = VectorElement("Lagrange", mesh.ufl_cell(), v_lagrange_order)
Q = FiniteElement("Lagrange", mesh.ufl_cell(), p_lagrange_order)
...
def get_u_exact(t):
	return Expression(('F*cos(x[0])*sin(x[1])','-F*cos(x[1])*sin(x[0])'),t=t,F=FF(t),degree = v_lagrange_order)
def get_p_exact(t):
	return Expression('-0.25*F*F*(cos(2*x[0])+cos(2*x[1]))',t = t, F=FF(t),degree =p_lagrange_order )
